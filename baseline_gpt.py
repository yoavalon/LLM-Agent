...
output_loaction = '/home/algo/code/alpha/gpt/agent/sequences/7'
np.random.seed(0) 

key = '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'
...
def inference(first, second, number_completions) : 
    ...
    #prompt 1 
    request = f'Given programs {first} and {second}, transform the first program so that it becomes syntactically more similar to the second program, while retaining its semantics. Apply only one atomic transformations. Provide only the source code without your comments.'
    ...
    n = number_completions
    )

    #if model gpt-4, than this neeeds to be true !!! [4.5 perhaps???]
    isPreview = False 
